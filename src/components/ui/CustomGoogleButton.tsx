...
interface CustomGoogleButtonProps {
  /** "login" أو "register" — يحدد أي endpoint في الباك اند يتم استدعاؤه */
  mode: "login" | "register";
  /** رابط الباك اند الأساسي */
  backendUrl?: string;
}
...
const CustomGoogleButton: React.FC<CustomGoogleButtonProps> = ({ 
  ...
  const handleClick = () => {
    // التوجيه المباشر للباك اند — هو اللي هيعمل redirect لجوجل
    // الباك اند هيستخدم الـ mode عشان يعرف يرجع اليوزر على أي صفحة بعد كده
  ...
  const buttonText = mode === "register"
    ...
    : (language === "ar" ? "تسجيل الدخول باستخدام جوجل" : "Continue with Google");
