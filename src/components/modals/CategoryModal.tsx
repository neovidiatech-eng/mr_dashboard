...
import { getCategorySchema, CategoryFormData } from '../../lib/schemas/CategorySchema';
...
interface CategoryFormProps {
  ...
  onSave: (data: CategoryFormData) => boolean | Promise<boolean>;
  onCancel: () => void;
  title: string;
}

export default function CategoryForm({ initial, onSave, onCancel, title }: CategoryFormProps) {
  const { t, language } = useLanguage();
  const { register, handleSubmit, setValue, watch, reset, formState: { errors } } = useForm<CategoryFormData>({
    resolver: zodResolver(getCategorySchema(t)) as Resolver<CategoryFormData>,
    defaultValues: {
        ...
        <form onSubmit={handleSubmit(handleSave)} className="p-6 space-y-5 max-h-[80vh] overflow-y-auto no-scrollbar">
          <div>
            <label className="block text-sm font-medium text-gray-700 text-start mb-1.5">
              {language === 'ar' ? 'اسم التصنيف بالعربي' : 'Category Name (Arabic)'}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 text-start mb-1.5">
              {language === 'ar' ? 'اسم التصنيف بالإنجليزي' : 'Category Name (English)'}
            </label>
            <input
              type="text"
              {...register('name_en')}
              ...
              {COLORS.map(c => (
                ...
                >
                  <div className={`w-8 h-8 rounded-lg ${c.bg} flex items-center justify-center`}>
                    <Layers className={`w-4 h-4 ${c.icon}`} />
                  </div>
                </button>
              ))}
            </div>
