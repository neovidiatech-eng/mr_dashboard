...
export default function UpdateSubscriptionStatusModal({
  ...
  const text = {
    ...
    subtitle: {
      ar: "مراجعة وتحديث حالة طلب الاشتراك الخاص بالطالب",
      en: "Review and update student subscription request status",
    },
      ...
      <div className="relative bg-white rounded-[32px] shadow-2xl max-w-md w-full overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-6 no-scrollbar">
          ...
          <div className="bg-slate-50/50 rounded-3xl p-6 border border-slate-100 space-y-4">
            ...
            {/* Academic Rank & Stage from Student Registration */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 border-t border-slate-100/50 pt-4">
              {/* Rank */}
              <div className="p-3.5 bg-white rounded-2xl border border-slate-100 text-start shadow-sm">
                <div className="flex items-center gap-2 mb-1.5 text-amber-500 font-bold text-xs">
                  <Trophy size={14} />
              ...
              {/* Stage */}
              <div className="p-3.5 bg-white rounded-2xl border border-slate-100 text-start shadow-sm">
                <div className="flex items-center gap-2 mb-1.5 text-indigo-500 font-bold text-xs">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`flex items-center justify-center gap-2 py-4 rounded-2xl font-bold transition-all border-2 ${
                  status === "approved"
                    ? "bg-emerald-50 border-emerald-500 text-emerald-700 shadow-md shadow-emerald-100 scale-105"
              ...
              <button
                ...
                className={`flex items-center justify-center gap-2 py-4 rounded-2xl font-bold transition-all border-2 ${
                  status === "rejected"
                    ? "bg-red-50 border-red-500 text-red-700 shadow-md shadow-red-100 scale-105"
                    : "bg-white border-slate-100 text-slate-400 hover:border-slate-200"
                }`}
              >
                <XCircle size={18} />
        ...
        <div className="p-8 border-t border-slate-100 bg-slate-50/30 flex flex-col gap-3">
          <button
            ...
            className={`w-full py-4 rounded-2xl font-black text-sm uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-lg active:scale-[0.98] disabled:opacity-50 disabled:grayscale ${
              status === "approved"
