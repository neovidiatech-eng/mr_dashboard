...
export default function ViewStudentModal({ isOpen, onClose, studentData: initialStudentData }: ViewStudentModalProps) {
  ...
  const coursePurchases: any[] =
    (studentData?.coursePurchases && studentData.coursePurchases.length > 0)
      ? studentData.coursePurchases
      : (initialStudentData?.coursePurchases && initialStudentData.coursePurchases.length > 0)
      ? initialStudentData.coursePurchases
      : (studentData as any)?.course_purchases || (initialStudentData as any)?.course_purchases
      || (studentData as any)?.user?.coursePurchases || (initialStudentData as any)?.user?.coursePurchases
      || [];

  const getCourseImageUrl = (imagePath?: string) => {
    if (!imagePath) return null;
    if (imagePath.startsWith('http://') || imagePath.startsWith('https://') || imagePath.startsWith('data:')) {
      return imagePath;
    }
    const baseUrl = ((import.meta.env.VITE_API_URL as string) || 'https://mr-dashboard-lyart.vercel.app').replace(/\/api\/?$/, '');
          ...
          <div className="mt-8 space-y-4">
            <div className="flex items-center justify-between">
              <h4 className="text-[11px] font-black text-gray-400 uppercase tracking-[2px]">
                {t('purchasedCourses') || (language === 'ar' ? 'الكورسات المشتراة' : 'Purchased Courses')}
              </h4>
              {coursePurchases && coursePurchases.length > 0 && (
                <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-100">
                  {coursePurchases.length} {language === 'ar' ? (coursePurchases.length > 1 ? 'كورسات' : 'كورس') : (coursePurchases.length > 1 ? 'Courses' : 'Course')}
                </span>
              )}
            </div>
            ...
            {coursePurchases && coursePurchases.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                {coursePurchases.map((purchase: any) => {
                  ...
                  const title = language === 'ar'
                    ? (course.title_ar || course.title_en || course.title || 'كورس')
                    : (course.title_en || course.title_ar || course.title || 'Course');
                  const price = course.price;
                  const purchaseDate = purchase.purchasedAt || purchase.createdAt;
                  const imgUrl = getCourseImageUrl(course.image);

                  return (
                    <div
                      key={purchase.id || Math.random()}
                      className="bg-white hover:bg-emerald-50/20 rounded-2xl p-3.5 border border-gray-100 transition-all hover:border-emerald-200 flex items-start gap-3 shadow-xs"
                    >
                      <div className="w-11 h-11 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center shrink-0 font-bold overflow-hidden border border-emerald-100/80">
                        {imgUrl ? (
                          <img
                            src={imgUrl}
                            alt={title}
                      ...
                      <div className="min-w-0 flex-1">
                        <h5 className="text-xs font-bold text-gray-900 leading-snug truncate" title={title}>
                          {title}
                        </h5>

                        <div className="flex items-center gap-2.5 mt-2 flex-wrap text-[11px]">
                          {price !== undefined && price !== null && (
                            <span className="font-extrabold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100/60">
                              {price} {language === 'ar' ? 'ج.م' : 'EGP'}
                            </span>
                          )}
                          {purchaseDate && (
                            <span className="flex items-center gap-1 text-[10px] font-semibold text-gray-400">
                              <Calendar className="w-3 h-3 text-gray-400" />
                              {new Date(purchaseDate).toLocaleDateString(language === 'ar' ? 'ar-EG' : 'en-US')}
                            </span>
                          )}
                        </div>
                      </div>
              ...
              <div className="bg-gray-50/50 rounded-2xl p-6 border border-gray-100 border-dashed flex flex-col items-center justify-center text-center">
                ...
                <p className="text-sm font-bold text-gray-500">
                  {t('noPurchasedCourses') || (language === 'ar' ? 'لا يوجد كورسات مشتراة' : 'No purchased courses')}
                </p>
              </div>
            )}
          </div>
