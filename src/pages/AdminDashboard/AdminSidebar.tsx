              ...
              {hasRouteAccess('posts') && (
                <NavLink
                  ...
                  className={({ isActive }) => `
                    ...
                    ${isActive ? 'bg-primary-light text-primary' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50'}
                  `}
                  title={isCollapsed ? t('sidebar_posts', 'Posts') : ''}
                >
                  <FileText className={`w-5 h-5 flex-shrink-0 transition-all ${isCollapsed ? 'mx-auto' : ''}`} />
                  {!isCollapsed && <span className={`text-sm flex-1 ${language === 'ar' ? 'text-right' : 'text-left'}`}>{t('sidebar_posts', 'Posts')}</span>}
                </NavLink>
              )}

              {hasRouteAccess('notifications') && (
                <NavLink
              ...
              {hasRouteAccess('ranks') && (
                <NavLink
                  ...
                  className={({ isActive }) => `
                    ...
                    ${isActive ? 'bg-primary-light text-primary' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50'}
                  `}
                  title={isCollapsed ? t('sidebar_levels', 'Levels') : ''}
                >
                  <Trophy className={`w-5 h-5 flex-shrink-0 transition-all ${isCollapsed ? 'mx-auto' : ''}`} />
                  {!isCollapsed && <span className={`text-sm flex-1 ${language === 'ar' ? 'text-right' : 'text-left'}`}>{t('sidebar_levels', 'Levels')}</span>}
                </NavLink>
              )}
            ...
            {hasRouteAccess('offline') && (
              <NavLink
                ...
                className={({ isActive }) => `
                  ...
                  ${isActive ? 'bg-primary-light text-primary' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50'}
                `}
                title={isCollapsed ? t('sidebar_offline', 'Offline Students') : ''}
              >
                <QrCode className={`w-5 h-5 flex-shrink-0 transition-all ${isCollapsed ? 'mx-auto' : ''}`} />
                {!isCollapsed && <span className={`text-sm flex-1 ${language === 'ar' ? 'text-right' : 'text-left'}`}>{t('sidebar_offline', 'Offline Students')}</span>}
              </NavLink>
            )}
