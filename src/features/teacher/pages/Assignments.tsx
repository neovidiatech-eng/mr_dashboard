...
export default function Assignments() {
  ...
  const text = {
    ...
    subtitle: {
      ar: "إنشاء وتتبع الواجبات المسندة للطلاب",
      en: "Create and track student assignments",
    },
    search: {
    ...
    deleteMessage: {
      ar: "هل أنت متأكد من حذف هذا الواجب؟",
      en: "Are you sure you want to delete this assignment?",
    },
    noResults: {
      ar: "لا توجد واجبات تطابق بحثك",
      en: "No assignments match your search",
    },
  ...
  const filteredAssignments = useMemo(() => {
    ...
    return assignments.filter((assignment) => {
      ...
      const title =
        assignment.title || assignment.title_en || assignment.title_ar || "";
      const description =
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      case "pending":
        return "bg-amber-50 text-amber-600 border-amber-200/50 shadow-[0_2px_10px_-3px_rgba(245,158,11,0.2)]";
      case "submitted":
        return "bg-primary-light text-primary border-primary/20 shadow-sm";
      case "completed":
        return "bg-emerald-50 text-emerald-600 border-emerald-200/50 shadow-[0_2px_10px_-3px_rgba(16,185,129,0.2)]";
      default:
  ...
  const columns = [
        ...
        const desc =
          language === "ar"
            ...
            : record.description_en ||
              ...
              record.description;
        return (
          <span className="text-sm text-gray-500 block max-w-[200px] truncate">
            {desc || "—"}
          </span>
        );
      },
    ...
    },
    {
      ...
      render: (_: any, record: HomeworkItem) => (
        <span
          className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold border uppercase tracking-widest ${getStatusStyle(record.status)}`}
        >
          ...
          {record.attachments && record.attachments.length > 0 ? (
            record.attachments.map((file, idx) => (
              ...
              >
                ...
                <span className="max-w-[100px] truncate uppercase tracking-tighter">
                  {file.name}
                </span>
        ...
        const items = [
          {
            ...
            label: (
              <span className="flex items-center gap-2 text-xs font-bold text-gray-700">
                <Edit2 className="w-3.5 h-3.5" /> {text.edit[language]}
              </span>
            ),
          ...
          },
          {
            ...
            label: (
              <span className="flex items-center gap-2 text-xs font-bold text-red-600">
                <Trash2 className="w-3.5 h-3.5" /> {text.delete[language]}
              </span>
            ),
            danger: true,
  ...
  return (
    <div
      className="space-y-8 max-w-[1500px] mx-auto p-4 custom-scrollbar"
        ...
        {stats.map((stat) => (
          ...
          >
            <div className="flex items-center justify-between mb-4 relative z-10">
              <div
                className={`p-3.5 rounded-2xl ${stat.iconBg} transition-all group-hover:scale-110 group-hover:rotate-6 duration-300`}
              >
            ...
            <div
              className={`absolute -right-4 -bottom-4 w-24 h-24 rounded-full opacity-10 ${stat.iconBg} group-hover:scale-150 transition-transform duration-500`}
            />
          </div>
        ))}
      </div>
        ...
        {!isLoading && filteredAssignments.length > 0 && (
          <div className="p-4 border-t border-gray-50 flex items-center justify-between">
            <span className="text-xs text-gray-400 font-bold ml-2">
              ...
              {language === "ar" ? "من" : "of"} {filteredAssignments.length}
            </span>
