...
// Hooks
import {
  useGetAllLiveSessions,
  useDeleteLiveSession,
  useStartExistingLiveSession,
  useEndExistingLiveSession,
  useJoinLiveSession,
...
export default function LiveSessions() {
  ...
  // Queries & Mutations
  const { data: responseData, isLoading } = useGetAllLiveSessions(
    currentPage,
    itemsPerPage,
    searchTerm,
  );
  ...
  const totalItems =
    responseData?.data?.pagination?.totalItems ||
      ...
      onSuccess: () => {
        joinLive(record.id, {
          onSuccess: (joinRes: any) => {
            const roomName =
              joinRes?.data?.roomName || joinRes?.roomName || record.roomName;
  ...
  const handleJoinLiveSession = (record: LiveSessionType) => {
    joinLive(record.id, {
      onSuccess: (res: any) => {
        const roomName =
          res?.data?.roomName || res?.roomName || record.roomName;
    ...
    },
    {
      ...
      render: (_: unknown, record: LiveSessionType) => (
        <div className="flex items-center gap-2">
          <span
            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs uppercase ${getBadgeStyle(record.status)}`}
          >
            {record.status === "live" && (
            ...
            ? [
                {
                  ...
                  label: (
                    <span className="flex items-center gap-2 text-xs font-bold text-emerald-600">
                      ...
                      {language === "ar" ? "انضمام للبث" : "Join Stream"}
                    </span>
                  ),
                  danger: false,
                  onClick: () => handleJoinLiveSession(record),
  ...
  return (
    <div
      className="space-y-6 max-w-[1500px] mx-auto p-2 flex flex-col"
        ...
        <div className="px-8 pb-4 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="relative flex-1 w-full max-w-md">
            ...
            <input
              ...
              placeholder={
                language === "ar"
                  ? "بحث باسم الجلسة أو المرحلة أو الخطة..."
                  : "Search sessions..."
              }
              value={searchTerm}
              onChange={(e) => {
